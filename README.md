...
- **Tooling:** strict TypeScript, ESLint, Prettier, Vitest, Testing Library, Playwright E2E, coverage and TypeDoc.
...
Playwright covers browser-level behavior that jsdom cannot validate well:

- production Vite preview boot and critical console-error smoke checks
- desktop and mobile Chromium rendering
- accessible workbench tab switching across table, kanban and calendar
- persisted dark mode behavior across reloads
- inline edit, pending operation and sync/conflict flows in a real browser
- command palette keyboard flow and dialog semantics
...
- `npm run test:e2e` - run Playwright against a production Vite preview.
...
- `npm run ci` - run typecheck, lint, coverage, build, docs, Playwright E2E and Pages report assembly locally.
